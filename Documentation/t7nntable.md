...
This allows one to build very rich architectures:
  ...
  * `table` Container Modules encapsulate sub-Modules:
    * [`ConcatTable`](#nn.ConcatTable): applies each member module to the same input     [`Tensor`](t7tensor.md#tensor) and outputs a `table`;
  ...
  * `Table` of Criteria:
    * [`CriterionTable`](#nn.CriterionTable): wraps a [Criterion](t7nncriterion.md#nn.Criterion) so that it can accept a `table` of inputs.

`table`-based modules work by supporting `forward()` and `backward()` methods that can accept `table`s as inputs.
It turns out that the usual [`Sequential`](t7nncontainers.md#nn.Sequential) module can do this, so all that is needed is other child modules that take advantage of such `table`s.
...
`ConcatTable` is a container module that applies each member module to the same input [`Tensor`](t7tensor.md#tensor) or `table`.
...
Creates a module that takes a [`Tensor`](t7tensor.md#tensor) as input and outputs several `table`s, splitting the `Tensor` along the specified `dimension`.
...
Creates a module that takes a `table` of `Tensor`s as input and outputs a [`Tensor`](t7tensor.md#tensor) by joining them together along dimension `dimension`.
...
This can be either a `table` or a [`Tensor`](t7tensor.md#tensor).
...
The elements can be either a `table` or a [`Tensor`](t7tensor.md#tensor).
