...
child classes, like [DiskFile](t7diskfile.md),
[MemoryFile](t7emoryfile.md) and [PipeFile](t7pipefile.md).
...
and returned in a new [Storage](t7storage.md) of that particular type.
...
Interestingly, if the `File` is a [MemoryFile](t7memoryfile.md), it allows
...
written on disk: if writing a [Storage](t7storage.md), a space will be
