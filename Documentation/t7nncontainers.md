...
See also the [Table Containers](#nn.TableContainers) for manipulating tables of [Tensors](t7tensor.md).

<a name="nn.Container"></a>
## Container ##

This is an abstract [Module](t7nnmodule.md#nn.Module) class which declares methods defined in all containers.
It reimplements many of the Module methods such that calls are propagated to the
contained modules. For example, a call to [zeroGradParameters](t7nnmodule.md#nn.Module.zeroGradParameters)
...
Creates a container module that applies its `ith` child module to the  `ith` slice of the input Tensor by using [select](t7tensor.md#tensor-selectdim-index)
...
The module is particularly useful for concatenating the output of [Convolutions](t7nnconvolution.md)
...
## Table Containers ##
While the above containers are used for manipulating input [Tensors](t7tensor.md), table containers are used for manipulating tables :
 * [ConcatTable](table.md#nn.ConcatTable)
 * [ParallelTable](table.md#nn.ParallelTable)

These, along with all other modules for manipulating tables can be found [here](t7nntable.md).
