...
for a training a neural network. Modules are [serializable](t7serialization.md#serialization).
...
[Tensors](t7tensor.md). However, some special sub-classes
like [table layers](t7nntable.md#nn.TableLayers) might expect something else. Please,
...
Note that this function if called on a [Container](t7nncontainers.md#nn.Containers)
...
like [table layers](t7nntable.md#nn.TableLayers) contain something else. Please,
refer to each module specification for further information.
...
### training() ###
This sets the mode of the Module (or sub-modules) to `train=true`. This is useful for modules like [Dropout](t7nnsimple.md#nn.Dropout) or [BatchNormalization](t7nnsimple.md#nn.BatchNormalization) that have a different behaviour during training vs evaluation.

<a name="nn.Module.evaluate"></a>
### evaluate() ###
This sets the mode of the Module (or sub-modules) to `train=false`. This is useful for modules like [Dropout](t7nnsimple.md#nn.Dropout) or [BatchNormalization](t7nnsimple.md#nn.BatchNormalization) that have a different behaviour during training vs evaluation.
